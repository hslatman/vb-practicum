...
public final class Checker implements Visitor {
  ...
  public Object visitRepeatUntilCommand(RepeatUntilCommand ast, Object o){
	  ast.C.visit(this, null); //visit the Command
	  TypeDenoter eType = (TypeDenoter) ast.E.visit(this, null); //visit the Expression
	  if (! eType.equals(StdEnvironment.booleanType)){ //test if the Expression is a boolean
	      reporter.reportError("Boolean expression expected here", "", ast.E.position); //report error if not boolean
	  }
	  return null;
  }
  
  //   http://www.comsci.us/cd/index.html
  
  public Object visitCaseCommand(CaseCommand ast, Object o){ //visit the CaseCommand
	  TypeDenoter eType = (TypeDenoter) ast.E.visit(this, null); //visit the Expression
	  if(! eType.equals(StdEnvironment.integerType)){ //test if it's indeed an Integer
		  reporter.reportError("Integer expression expected here", "", ast.E.position); //report error if the Expression is not an Integer
	  }
	  ArrayList<String> list = new ArrayList<String>(); //create a new list, that will contain the cases (at least, the case identifiers) to check for duplicates
	  ast.CASE.visit(this, list); //visit the abstract Case class. Results in visiting a CaseSingle or CaseMultiple. See below.
	  ast.C.visit(this, null); //visit the Command
	  return null;
  }
  
  @SuppressWarnings("unchecked")
  public Object visitCaseSingle(CaseSingle ast, Object o){
	  TypeDenoter ilType = (TypeDenoter) ast.IL.visit(this, null); //visit the IntegerLiteral for the CaseSingle
	  if(! ilType.equals(StdEnvironment.integerType)) { //check if it's indeed an Integer
		  reporter.reportError("Case identifier should be Integer Literal.", ast.IL.spelling, ast.position); //report error when it's not
	  }
	  ast.C.visit(this, null); //Visit the Command that belongs to this CaseSingle
	  ((ArrayList<String>) o).add(ast.IL.spelling); //add it to the ArrayList, such that it now contains this identifier for a case (we are the last one)
	  return null;
  }
  
  @SuppressWarnings("unchecked")
  public Object visitCaseMultiple(CaseMultiple ast, Object o){
	  TypeDenoter ilType = (TypeDenoter) ast.IL.visit(this, null); //visit the IntegerLiteral for this CaseMultiple
	  if(! ilType.equals(StdEnvironment.integerType)) { //check if it's indeed an IntegerLiteral
		  reporter.reportError("Case identifier should be Integer Literal.", ast.IL.spelling, ast.position); //report the error when not an Integer
	  }
	  ast.C.visit(this, null); //visit the Command for this CaseMultiple
	  ast.CASE.visit(this, o); //visit the CASE (which could be another CaseMultiple OR a CaseSingle). Use o to get its identifier added.
	  if (((ArrayList<String>) o).contains(ast.IL.spelling)){ //check to see if the identifier is already in the ArrayList (duplicate cases)
		  reporter.reportError(ast.IL.spelling + " case was already defined.", ast.IL.spelling, ast.IL.position); //error when it's already there
	  }
	  ((ArrayList<String>) o).add(ast.IL.spelling); //add the case identifier for this CaseMultiple
	  return null;
  }
